...
class FCN_LectureNet_Image_Binarizer(FCN_LectureNet_Binarizer):

    def __init__(self, lecture_net):
        super().__init__(lecture_net)

    def handleImage(self, image):

        pil_image = Image.fromarray(cv2.cvtColor(image, cv2.COLOR_RGB2BGR))
        binary, _, _ = self.lecture_net.binarize(pil_image, return_others=True, force_binary=True)
    ...
    def getWorkName(self):
        return "FCN_LectureNet Image Binarizer"

if __name__ == "__main__":

    image_binarizer = FCN_LectureNet_Image_Binarizer